...
BRUTOS_UNIFICADO_REL="ciclos/${SESSION_MARCA}_ATE_${JANELA_FIM_FMT}_emails_brutos_unificado.csv"
...
echo "Arquivo bruto unificado (6h): $BRUTOS_UNIFICADO_REL" >> "$SESSAO_LOG"
...
print(f"BRUTOS_UNIFICADO_TOTAL={len(combinados)}")
PY
  else
  ...
  while true; do
    ...
    "$PY" buscar_urls.py --acumular >> "$LOG" 2>&1 &
    pid_busca=$!
